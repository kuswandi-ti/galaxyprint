...
class Keluar_Scrap extends MX_Controller {

    public $data;

    var $module                 = 'produksi';
    var $title                  = 'Daftar Pengeluaran Barang Sisa (Scrap)';
    var $file_name              = 'keluar_scrap';
    ...
    public function index() {
        $data = array(
            ...
            'get_master_scrap'  => $this->main->get_master_scrap(),
    ...
    function show_detail() {
        ...
        if ($data->num_rows() > 0) {
            ...
            foreach ($data->result() as $row) {
                ...
                echo    '<td class="text-right"><label>'.$row->total_detail.'</label><input type="hidden" name="total_detail_detail[]" class="" readonly value="'.$row->total_detail.'"></td>';
    ...
    function get_info_scrap() {
        $kode_barang = $this->input->post('kode_barang');
        $res = $this->db->where('kode_barang', $kode_barang)->get($this->table_scrap);
        if ($res->num_rows() > 0) {
