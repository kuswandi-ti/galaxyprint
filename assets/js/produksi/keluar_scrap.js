...
$(document).ready(function() {
    ...
    $("body").on("change", "#nama_barang", function() {
        $.ajax({
            url: base_url + "produksi/keluar_scrap/get_info_scrap",
            data: {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": base_url+'produksi/keluar_scrap/ajax_list',
            "type": "POST"
        },
...
function add() {
    ...
    $('.modal-title').text('Tambah Pengeluaran Barang Sisa (Scrap)'); // Set Title to Bootstrap modal title
}

function show_detail() {
    var id_header = $("#id_hidden").val();
    $.ajax({
        url : base_url+'produksi/keluar_scrap/show_detail/?id_header='+id_header,
        async : false,
        success : function(data) {
...
function edit(id) {
    ...
    $.ajax({
        url : base_url+"produksi/keluar_scrap/ajax_edit/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data) { 
            ...
            $("#penerima").data('select2').trigger('select', {
            ...
            $('[name="total_scrap"]').val(data.total);
            show_detail();
            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Edit Pengeluaran Barang Sisa (Scrap)'); // Set title to Bootstrap modal title 
        },
...
function save() {
    ...
    if(save_method == 'add') {
        url = base_url+"produksi/keluar_scrap/ajax_add";
    } else {
        url = base_url+"produksi/keluar_scrap/ajax_update";
    }
...
function hapus(id) {
    if (confirm('Are you sure delete this data?')) {
        ...
        $.ajax({
            url : base_url+"produksi/keluar_scrap/ajax_delete/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data) {
